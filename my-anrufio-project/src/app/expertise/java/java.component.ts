...
@Component({
  selector: 'app-java',
  imports: [ExpertiseTechnologiesComponent, HeaderComponent, FooterComponent, ServiceForExpertiseComponent, FullStackDevelopmentComponent, QualityDrivenProficiencyComponent, TestimonialComponent, FaqComponent, BookCallComponent, BookCallComponent, RecentBlogComponent,BenefitsComponent],
...
export class JavaComponent {
  ...
  coreCompetencies: any;
    ...
    expertise:any
  ngOnInit(): void {
    this.http.get('assets/json/nodejs.json').subscribe({
      next: (data: any) => {
    ...
    this.http.get('assets/json/nodejs.json').subscribe((response: any) => {
      this.expertise = response;
    });

    this.http.get<any>('assets/json/nodejs.json').subscribe((response: { pwaServices: any; }) => {
      this.pwaService = response.pwaServices; // Assign the entire pwaServices object
    });
    this.http.get<any>('assets/json/nodejs.json').subscribe((response: { deployment: any; }) => {
      this.deploymentDatas = response.deployment;
    });
    this.http.get<any>('assets/json/nodejs.json').subscribe(
