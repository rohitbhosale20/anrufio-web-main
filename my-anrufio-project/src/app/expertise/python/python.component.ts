...
faqData!: FAQSection;
  ...
  coreCompetencies: any;
    ...
    expertise:any
  ngOnInit(): void {
    this.http.get('assets/json/python.json').subscribe({
      next: (data: any) => {
    ...
    this.http.get('assets/json/python.json').subscribe((response: any) => {
      this.expertise = response;
    });

    this.http.get<any>('assets/json/python.json').subscribe((response: { pwaServices: any; }) => {
      this.pwaService = response.pwaServices; // Assign the entire pwaServices object
    });
    this.http.get<any>('assets/json/python.json').subscribe((response: { deployment: any; }) => {
      this.deploymentDatas = response.deployment;
    });
    this.http.get<any>('assets/json/python.json').subscribe(
