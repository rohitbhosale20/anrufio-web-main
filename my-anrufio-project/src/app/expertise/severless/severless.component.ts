...
@Component({
  ...
  templateUrl: './severless.component.html',
  styleUrl: './severless.component.css'
})
export class SeverlessComponent {
faqData!: FAQSection;
  benefitsData: any;
  showButton!:boolean
  bookCall: any;
  ...
  coreCompetencies: any;
    ...
    expertise:any
  ngOnInit(): void {
    this.http.get('assets/json/serverless.json').subscribe({
      next: (data: any) => {
    ...
    this.http.get('assets/json/serverless.json').subscribe((response: any) => {
      this.expertise = response;
    });

    this.http.get<any>('assets/json/serverless.json').subscribe((response: { pwaServices: any; }) => {
      this.pwaService = response.pwaServices; // Assign the entire pwaServices object
    });
    this.http.get<any>('assets/json/serverless.json').subscribe((response: { deployment: any; }) => {
      this.deploymentDatas = response.deployment;
    });
    this.http.get<any>('assets/json/serverless.json').subscribe(
